  ...
  for(i=0; z[i]; i++){
    ...
    if( strncmp(&z[i],"%endif",6)==0 && isspace(z[i+6]) ){
      ...
      if( exclude ){
        ...
        exclude = 1;
        for(k=0; k<nDefine; k++){
          if( strncmp(azDefine[k],&z[j],n)==0 && strlen(azDefine[k])==(unsigned)n ){
            exclude = 0;
            break;
          }
        }
...
struct lemon *gp;
{
  ...
  if( fread(filebuf,1,filesize,fp)!=(unsigned)filesize ){
...
PRIVATE char *append_str(char *zText, int n, int p1, int p2){
  ...
  if( n+sizeof(zInt)*2+used >= (unsigned)alloced ){
...
#if 0           /* HACK (daveb): do not try to detect changes in .h file */
